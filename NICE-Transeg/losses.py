...
def NJD_new(disp):
    ...
    # gradx = torch.nn.Conv3d(1, 1, kernel_size=(3, 1, 1), padding='same')
...
def NJD_old(disp):
    ...
    gradx_disp_torch = torch.stack([F.conv3d(disp_torch[:, 0, :, :, :], gradx_torch, padding='valid'),
                           F.conv3d(disp_torch[:, 1, :, :, :], gradx_torch, padding='valid'),
                           F.conv3d(disp_torch[:, 2, :, :, :], gradx_torch, padding='valid')], axis=1)
